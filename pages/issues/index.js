            ...
            issues.map((issue) => (
              <div key={issue._id} className="p-4 space-y-2">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  >
                    {issue.isOverdue
                      ? t(router.locale, 'overdue').toLowerCase()
                ...
                <div className="text-xs text-gray-600 flex flex-wrap gap-3">
                  ...
                  {issue.location && (
                    <span>{t(router.locale, 'location')}: {issue.location}</span>
                  )}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => handleVote(issue._id)}
                    disabled={
                      issue.status === 'resolved' || busyIssueId === issue._id
                    }
                    className={`px-3 py-1 rounded text-sm ${
                      issue.hasVoted
...
//   const apiUrl = mine === '1' ? '/api/issues?mine=1' : '/api/issues';
...
//             {mine === '1' ? 'My Issues' : t(router.locale, 'issueTracker')}
//           </h2>
...
//                 <div className="text-xs text-gray-600 flex flex-wrap gap-3">
//                   <span>{t(router.locale, 'category')}: {issue.category}</span>
//                   {issue.location && <span>{t(router.locale, 'location')}: {issue.location}</span>}
//                   {issue.deadline && (
//                     <span>
//                       {t(router.locale, 'resolutionDeadline')}:{' '}
//                       {new Date(issue.deadline).toLocaleDateString()}
